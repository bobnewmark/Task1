...
// TODO: add tests here
        ...
        Result toStringResult = new JUnitCore().run(ToStringTest.class);
        if (!toStringResult.wasSuccessful()) {
            List <Failure> failureList1 = toStringResult.getFailures();
            failureList1.forEach(failure -> System.out.println("Failure: " + failure.getMessage()));
        }

        Result discountResult = new JUnitCore().run(DiscountTest.class);
        if (!discountResult.wasSuccessful()) {
            List <Failure> failureList2 = discountResult.getFailures();
            failureList2.forEach(failure -> System.out.println("Failure: " + failure.getMessage()));
        }
    }
